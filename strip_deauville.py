...
def run_deauville_stripping():
    ...
    for i, text in enumerate(findings):
        ...
        if type(text) is not str:
            ...
            ds = highest_deauville(text_filt)
...
def run_split_reports_according_to_ds(options='binary'):
    ...
    for cut in cut_points:
        ...
        save_name = 'ds' + str(cut).replace('[','').replace(']','').replace(',','').replace(' ', '') + '_' + read_file
...
def interpretability_replace_key_sentences_in_test_dataset(test_fract = 0.2,
    ...
    df_test_replaced_doc2vec.to_csv(os.path.join(report_direct, 'DECOYS_' + 'doc2vec_' + str(len(report_files)) + '_classes.csv'))
